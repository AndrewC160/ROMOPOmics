...
sum_m_data  <- gds_list$merged_metadata
values      <- sum_m_data %>% 
...
m_data      <- lapply(gds_list[grepl("^GDS",names(gds_list))], function(x) x$metadata)

gds_cols    <- lapply(m_data,colnames) %>% 
                unlist() %>%
...
write.table(sum_m_data,file = file.path(base_dir,"geo_interface/example_metadata_table.tsv"),sep = "\t",row.names = FALSE,quote = FALSE)
values      <- sum_m_data %>% 
                summarize_all(function(x) list(x)) %>% 
                pivot_longer(cols = everything()) %>%
                rowwise() %>%
